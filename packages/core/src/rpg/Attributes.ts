...
export function gerarAtributosGeneticos(
  atributosPai: Atributos,
  atributosMae: Atributos,
  semente: number,
): Atributos {
  const pseudoAleatorio = (s: number): number => {
    let temp = s;
    temp += 0x6D2B79F5;
    let z = temp;
  ...
  const calcularAtributo = (pai: number, mae: number, offset: number): number => {
