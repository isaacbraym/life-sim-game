...
function desenharPathBezier(gfx, path, offsetX, offsetY) {
    const ox = offsetX;
    const oy = offsetY;
    const e = path.esq;
    const d = path.dir;
...
export function desenharSilhueta(gfx, esqueleto, offsetX, offsetY) {
    ...
    gfx.stroke();
    // --- perna direita ---
    const pathPernaD = gerarPathPerna(p('hip_R'), p('knee_R'), { x: p('hip_R').x, y: p('knee_R').y + 42 }, PERFIL_PERNA_PADRAO, true);
    ...
    gfx.bezierCurveTo(offsetX + quadrilL.x - 16, offsetY + ombrosY + 10, offsetX + p('shoulder_L').x - 2, offsetY + ombrosY, offsetX + p('shoulder_L').x + 4, offsetY + ombrosY);
    gfx.lineTo(offsetX + p('shoulder_R').x - 4, offsetY + ombrosY);
    gfx.bezierCurveTo(offsetX + p('shoulder_R').x + 2, offsetY + ombrosY, offsetX + quadrilR.x + 16, offsetY + ombrosY + 10, offsetX + quadrilR.x + 23, offsetY + pelvis.y);
    gfx.bezierCurveTo(offsetX + quadrilR.x + 18, offsetY + pelvis.y + 15, offsetX + quadrilR.x + 8, offsetY + pelvis.y + 20, offsetX + pelvis.x + 10, offsetY + pelvis.y + 18);
    ...
    gfx.bezierCurveTo(offsetX + quadrilL.x - 8, offsetY + pelvis.y + 20, offsetX + quadrilL.x - 18, offsetY + pelvis.y + 15, offsetX + quadrilL.x - 23, offsetY + pelvis.y);
    gfx.closePath();
    gfx.fill({ color: COR_PELE });
    gfx.stroke();
    // --- pescoço ---
