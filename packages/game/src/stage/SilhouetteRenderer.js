import { gerarPathBraco, gerarPathPerna, PERFIL_BRACO_PADRAO, PERFIL_PERNA_PADRAO, } from '@core/silhouette';
const COR_PELE = 0xf5c5a3;
const COR_SOMBRA = 0xe8a882;
const COR_CONTORNO = 0x2a1a0e;
const ESPESSURA_CONTORNO = 1.2;
function desenharElipseInclinada(gfx, cx, cy, rx, ry, angle) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    const m = 0.552284749831;
    const tx = (px, py) => cx + px * cos - py * sin;
    const ty = (px, py) => cy + px * sin + py * cos;
...
function desenharPathBezier(gfx, path, ox, oy) {
    const e = path.esq;
    const d = path.dir;
    ...
    gfx.bezierCurveTo(ox + d.p0.x + (d.p0.x - e.p2.x) * 0.3, oy + d.p0.y + (d.p0.y - e.p2.y) * 0.3, ox + d.p0.x, oy + d.p0.y, ox + d.p0.x, oy + d.p0.y);
...
export function desenharSilhueta(gfx, esqueleto, offsetX, offsetY) {
    gfx.clear();
    const p = (id) => esqueleto.posicaoMundialDe(id);
    ...
    gfx.bezierCurveTo(rootX - 25, offsetY + neck.y + 10, ombroEsqX, offsetY + sL.y - 10, ombroEsqX, offsetY + sL.y + 5);
    // Lado esquerdo: Ombro para quadril
    gfx.bezierCurveTo(ombroEsqX, offsetY + cinturaY - 10, cinturaEsqX, offsetY + cinturaY, quadrilEsqX, offsetY + pelvis.y);
    // Fundo (virilha)
    gfx.bezierCurveTo(quadrilEsqX + 5, offsetY + pelvis.y + 16, quadrilDirX - 5, offsetY + pelvis.y + 16, quadrilDirX, offsetY + pelvis.y);
    // Lado direito: Quadril para ombro
    gfx.bezierCurveTo(cinturaDirX, offsetY + cinturaY, ombroDirX, offsetY + cinturaY - 10, ombroDirX, offsetY + sR.y + 5);
    // Top right: Ombro para pescoço
    gfx.bezierCurveTo(ombroDirX, offsetY + sR.y - 10, rootX + 25, offsetY + neck.y + 10, rootX + 10, offsetY + neck.y + 10);
    // Conexão base do pescoço
    gfx.bezierCurveTo(rootX + 5, offsetY + neck.y + 12, rootX - 5, offsetY + neck.y + 12, rootX - 10, offsetY + neck.y + 10);
    gfx.closePath();
    gfx.fill({ color: COR_PELE });
    gfx.stroke();
...
//# sourceMappingURL=SilhouetteRenderer.js.map